...
uint16_t SET_SS_MEM_FLAG = 10;
uint16_t SET_HV_MEM_FLAG = 20;
uint16_t SET_LV_MEM_FLAG = 30;
uint16_t SET_HP_MEM_FLAG = 40;
uint16_t SET_OC_MEM_FLAG = 50;
uint16_t SET_UC_MEM_FLAG = 60;
uint16_t SET_UBV_MEM_FLAG = 70;
uint16_t RV_GAIN_MEM_FLAG = 80;
uint16_t YV_GAIN_MEM_FLAG = 90;
uint16_t BV_GAIN_MEM_FLAG = 100;
uint16_t RA_GAIN_MEM_FLAG = 110;
uint16_t YA_GAIN_MEM_FLAG = 120;
uint16_t BA_GAIN_MEM_FLAG = 130;
...
void setup() {
  ...
  EEPROM.get(SET_SS_MEM_FLAG, SET_SS);
...
void loop() {
  ...
  DELAY_TIME_CTD.PV = SET_SS;
  DELAY_TIME_CTD.process((AUTO_DELAY_FLAG == HIGH && PULSE == HIGH), AUTO_DELAY_FLAG == LOW);

  if (SCREEN == 1 && AUTO_MODE == HIGH && TRIP == LOW && MAN_CMD_FLAG == LOW) AUTO_DELAY_FLAG = HIGH;
  else AUTO_DELAY_FLAG = LOW;

  if (DELAY_TIME_CTD.Q) AUTO_CMD_FLAG = HIGH;
  else AUTO_CMD_FLAG = LOW;
  ...
  if (TRIP == HIGH) {
    LED_MAN_FLAG = LOW;
    if (LED == HIGH) LED_STOP_FLAG = LOW;
    else LED_STOP_FLAG = PULSE;
    LED_AUTO_FLAG = LOW;
    BUZ_FLAG = PULSE;
    TIMER = 4;
  } else if (SCREEN == 4) {
  ...
  if ((SCREEN == 2 && MENU_SCREEN_RTRIG.Q == HIGH) || MENU_EXIT_TIMER.Q == HIGH) {
    EEPROM.put(SET_SS_MEM_FLAG, SET_SS);
    ...
    EEPROM.put(SET_SS_MEM_FLAG, 2);
    EEPROM.put(SET_HV_MEM_FLAG, 500);
    EEPROM.put(SET_LV_MEM_FLAG, 300);
    EEPROM.put(SET_HP_MEM_FLAG, 3);
    EEPROM.put(SET_OC_MEM_FLAG, 60);
    EEPROM.put(SET_UC_MEM_FLAG, 5);
    EEPROM.put(SET_UBV_MEM_FLAG, 12);
    if (SET_HP_MAX <= 5) {
      EEPROM.put(RA_GAIN_MEM_FLAG, 650);  //690
      EEPROM.put(YA_GAIN_MEM_FLAG, 650);  //690
      EEPROM.put(BA_GAIN_MEM_FLAG, 650);  //690
    } else if (SET_HP_MAX == 10) {
  ...
  if (LED == HIGH && TRIP == LOW) {
    if (R_LED_FLAG == HIGH) module2.setSegments(0b00000001, 3);
         ...
         digitalWriteFast(R_LED, R_LED_FLAG),
         digitalWriteFast(Y_LED, Y_LED_FLAG),
         digitalWriteFast(B_LED, B_LED_FLAG);
